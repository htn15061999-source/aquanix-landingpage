...
const FOCUS = { 'feeding.png': '14% 50%', 'water-test.png': '74% 50%', 'farmer-phone.png': '20% 50%', 'harvest-scan.png': '30% 50%' };
...
const TRIM = { 'app-home.png': 'app-home-trim.png', 'app-account.png': 'app-account-trim.png' };
function Phone({ src, style }) {
  if (BEZELED.includes(src)) return <div style={{ position: 'relative', ...style }}><img src={P + TRIM[src]} alt="Màn hình app Aquanix" style={{ width: '100%', display: 'block', filter: 'drop-shadow(0 24px 40px rgba(11,42,51,.28))' }} /></div>;
  return <div style={{ borderRadius: 36, overflow: 'hidden', background: 'var(--ink)', padding: 6, boxShadow: '0 24px 48px rgba(11,42,51,.28)', ...style }}><img src={P + src} alt="Màn hình app Aquanix" style={{ width: '100%', display: 'block', borderRadius: 30 }} /></div>;
}
...
function PhotoPhone({ src, app, h = 440 }) {
  return <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,32%)', alignItems: 'center' }}>
    <div style={{ height: h, borderRadius: 'var(--radius-xl)', overflow: 'hidden', boxShadow: 'var(--shadow-card)' }}><img src={P + src} alt="" style={{ ...photo, objectPosition: FOCUS[src] || '50% 50%' }} /></div>
    <Phone src={app} style={{ marginLeft: '-12%', outline: BEZELED.includes(app) ? 'none' : '8px solid var(--surface)', position: 'relative' }} />
  </div>;
}
function Day() {
  const steps = [
    ['06:30', 'Cho ăn', 'Đổ cám như mọi sáng. Bấm "Cho ăn", chọn ao, nhập số kg.', 'App ghi giờ, ghi ao thay bạn. Cuối vụ, tổng cám từng ao tự cộng xong.', 'feeding.png', 'app-log.png', 'ok', 'Đã ghi · Ao A06 · 18 kg'],
    ['08:15', 'Đo nước', 'Đo pH, đo oxy như thường lệ. Nhập hai con số.', 'Oxy tụt, app báo ngay ở trang chủ và nói rõ việc cần làm — bật quạt, giảm cám.', 'water-test.png', 'app-home.png', 'stop', 'Ao A06 · Oxy 3,2 mg/L · thấp'],
    ['10:40', 'Dùng thuốc', 'Chọn tên thuốc, ghi liều, ghi ao.', 'App tự tính ngày được bán và nhắc bạn trước. Không lo bán sớm, không lo nhà máy trả cá.', 'farmer-phone.png', null, 'warn', 'Ao B02 · còn 6 ngày mới được bán'],
  ];
  return <section id="day"><div className="wrap sec">
    <Eyebrow>Một ngày ở ao</Eyebrow>
    <h2 className="t-display-l" style={{ margin: '0 0 var(--space-2)', maxWidth: 640, textWrap: 'pretty' }}>Việc vẫn làm như cũ. Chỉ thêm một chạm.</h2>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-8)' }}>
      {steps.map(([t, h, d1, d2, pic, app, st, badge], i) => <div key={t} className={'g2 feat' + (i % 2 ? ' rev' : '')} style={{ gap: 'var(--space-7)', alignItems: 'center' }}>
        <div>
          <div className="t-code" style={{ color: 'var(--brand-500)', marginBottom: 'var(--space-2)' }}>{t}</div>
          <h3 className="t-display-m" style={{ margin: '0 0 var(--space-3)', fontSize: 28, lineHeight: '36px' }}>{h}</h3>
          ...
          <StatusBadge status={st}>{badge}</StatusBadge>
        </div>
        {app ? <PhotoPhone src={pic} app={app} /> : <div style={{ height: 440, borderRadius: 'var(--radius-xl)', overflow: 'hidden', boxShadow: 'var(--shadow-card)' }}><img src={P + pic} alt="" style={{ ...photo, objectPosition: FOCUS[pic] || '50% 50%' }} /></div>}
      </div>)}
    </div>
  </div></section>;
}
  ...
  return <section id="sell" style={{ background: 'var(--surface-sunken)' }}><div className="wrap sec">
    <div className="g2" style={{ gap: 'var(--space-7)', alignItems: 'center' }}>
      <div>
        ...
        <h2 className="t-display-l" style={{ margin: '0 0 var(--space-4)', textWrap: 'pretty' }}>Nhập số cân. Hồ sơ cả vụ đã sẵn.</h2>
      ...
      <PhotoPhone src="harvest-scan.png" app="app-trace.png" h={460} />
    </div>
  </div></section>;
}
function Why() {
  ...
  return <section id="why"><div className="wrap sec">
    <Eyebrow>Bạn được gì</Eyebrow><h2 className="t-display-l" style={{ margin: '0 0 var(--space-6)', textWrap: 'pretty' }}>Ba điều hộ nuôi thấy ngay trong vụ đầu</h2>
    <div className="g3" style={{ gap: 'var(--space-4)' }}>{items.map(([t, d], i) => <div key={t} style={{ ...card, background: i === 2 ? 'var(--brand-50)' : 'var(--surface)', borderColor: i === 2 ? 'transparent' : 'var(--border)' }}><h3 className="t-display-m" style={{ margin: '0 0 var(--space-2)' }}>{t}</h3><p className="t-body" style={{ margin: 0, color: 'var(--ink-muted)', textWrap: 'pretty' }}>{d}</p></div>)}</div>
  </div></section>;
}
function Voice() {
  return <section style={{ position: 'relative', color: '#fff', overflow: 'hidden' }}>
    <img src={P + 'cooperative.png'} alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} />
    ...
    <div className="wrap sec" style={{ position: 'relative' }}>
      <blockquote style={{ margin: 0, maxWidth: 640, fontSize: 28, lineHeight: '38px', fontWeight: 600, textWrap: 'pretty' }}>"Cho ăn xong bấm một cái, khỏi phải nhớ. Sắp bán, app báo còn mấy ngày. Nhà máy nhận cá không hỏi thêm câu nào."</blockquote>
      <div style={{ marginTop: 'var(--space-5)' }}><div className="t-body-strong">Hộ nuôi ao A12</div><div className="t-caption" style={{ color: 'rgba(255,255,255,.75)' }}>Vùng nuôi Hải Vương 3 · trích dẫn minh hoạ</div></div>
    </div>
  </section>;
}
function Faq() {
...
function Download({ sent, onSend }) {
  return <section id="download"><div className="wrap sec">
    ...
    <h2 className="t-display-l" style={{ margin: '0 0 var(--space-2)', textWrap: 'pretty' }}>Bắt đầu vụ này cùng Aquanix</h2>
    ...
    <div className="g2" style={{ gap: 'var(--space-5)', alignItems: 'start' }}>
      <div style={{ ...card, background: 'var(--brand-50)', borderColor: 'transparent', display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) auto', gap: 'var(--space-5)', alignItems: 'center', flex: 1 }}>
          <Phone src="app-home.png" style={{ width: '100%', maxWidth: 190, justifySelf: 'center' }} />
      ...
      <form style={{ ...card, display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }} onSubmit={e => { e.preventDefault(); onSend(); }}>
        ...
        {sent ? <div style={{ padding: 'var(--space-4) 0' }}><StatusBadge status="ok">Đã nhận thông tin · Aquanix gọi lại trong hôm nay</StatusBadge></div> : <>
          ...
          <div className="g2" style={{ gap: 'var(--space-4)' }}><FieldInput label="Số điện thoại" inputMode="tel" required /><FieldInput label="Địa chỉ" hint="Xã, huyện, tỉnh hoặc tên vùng nuôi" /></div>
...
function Footer() {
  return <footer style={{ borderTop: '1px solid var(--border)' }}><div className="wrap" style={{ padding: 'var(--space-6) var(--space-5)', display: 'flex', alignItems: 'center', gap: 'var(--space-5)', flexWrap: 'wrap' }}>
    ...
    <span className="t-caption" style={{ color: 'var(--ink-muted)', marginLeft: 'auto' }}>Rô Phi Hải Vương VN</span>
  </div></footer>;
}
